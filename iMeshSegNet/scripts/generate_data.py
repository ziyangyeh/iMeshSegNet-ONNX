...
def assemble(mesh_path: str, label_path: str, out_dir: str, offset: int):
    ...
    vedo.io.write(result, os.path.join(out_dir, os.path.splitext(os.path.basename(mesh_path))[0]+".vtp"))
    ...
    flipped_result.to_vtp(os.path.join(out_dir, os.path.splitext(os.path.basename(mesh_path))[0]+"_f.vtp"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-in", "--input_dir", type=str, metavar="", help="Input Directory", required=True)
    parser.add_argument("-out", "--output_dir", type=str, metavar="", help="Output Directory", required=True)
    parser.add_argument("-off", "--offset", type=int, metavar="", help="Offset Value", default=0)
    parser.add_argument("-aug", "--augment", type=int, metavar="", help="Augment Value", default=20)
    parser.add_argument("-patch", "--patch_size", type=int, metavar="", help="Patch Size", default=7000)
    parser.add_argument("-cfg", "--cfg_file", type=str, metavar="", help="CFG File Path", required=True)
    ...
    print("Assembling...")
    Parallel(n_jobs=cpu_count())(delayed(assemble)(i, j, os.path.join(args.output_dir, "upper"), args.offset) for i,j in tzip(glob.glob(f"./{args.input_dir}/*/上/*.stl"),glob.glob(f"./{args.input_dir}/*/上/*.txt"), desc="upper"))
    Parallel(n_jobs=cpu_count())(delayed(assemble)(i, j, os.path.join(args.output_dir, "lower"), args.offset) for i,j in tzip(glob.glob(f"./{args.input_dir}/*/下/*.stl"),glob.glob(f"./{args.input_dir}/*/下/*.txt"), desc="lower"))

    print("Renaming...")
    for idx,file_name in enumerate(tqdm(glob.glob(f"{args.output_dir}/upper/*_f.vtp"), desc="upper")):
        os.rename(os.path.join(args.output_dir, "upper", os.path.basename(file_name)[:-6]+".vtp"), os.path.join(args.output_dir, "upper", "Sample_%04d.vtp" %idx))
        os.rename(os.path.join(args.output_dir, "upper", os.path.basename(file_name)), os.path.join(args.output_dir, "upper", "Sample_%04d.vtp" %(1000+idx)))
    for idx,file_name in enumerate(tqdm(glob.glob(f"{args.output_dir}/lower/*_f.vtp"), desc="lower")):
        os.rename(os.path.join(args.output_dir, "lower", os.path.basename(file_name)[:-6]+".vtp"), os.path.join(args.output_dir, "lower", "Sample_%04d.vtp" %idx))
        os.rename(os.path.join(args.output_dir, "lower", os.path.basename(file_name)), os.path.join(args.output_dir, "lower", "Sample_%04d.vtp" %(1000+idx)))
    ...
    Parallel(n_jobs=cpu_count())(delayed(augment)(item, os.path.join(aug_dir, "upper"), args.augment) for item in tqdm(glob.glob(f"{args.output_dir}/upper/*.vtp"), desc="upper"))
    Parallel(n_jobs=cpu_count())(delayed(augment)(item, os.path.join(aug_dir, "lower"), args.augment) for item in tqdm(glob.glob(f"{args.output_dir}/lower/*.vtp"), desc="lower"))

    print("Splitting Upper...")
    u_train_list, tmp_list = train_test_split(glob.glob(f"{aug_dir}/upper/*.vtp"), train_size=0.8)
    ...
    l_train_list, tmp_list = train_test_split(glob.glob(f"{aug_dir}/lower/*.vtp"), train_size=0.8)
    ...
    for jaw_type, abbr in zip(["upper", "lower"], ["u", "l"]):
        ...
        for item in ["train", "val", "test"]:
            ...
            data_group.create_dataset("label", data=np.zeros((length, 1, args.patch_size), dtype=np.int32), dtype="int32")
